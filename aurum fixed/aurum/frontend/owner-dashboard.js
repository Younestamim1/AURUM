/* ═══════════════════════════════════════════════
   AURUM — Owner Dashboard · owner-dashboard.js
...
if (user) {
  ...
  if (safe('navOwnerInitials'))  safe('navOwnerInitials').textContent  = user.initials   || 'OW';
  if (safe('navOwnerName'))      safe('navOwnerName').textContent      = (user.name || 'Owner').split(' ')[0];
  if (safe('sideOwnerInitials')) safe('sideOwnerInitials').textContent = user.initials   || 'OW';
  if (safe('sideOwnerName'))     safe('sideOwnerName').textContent     = user.name       || 'Owner';
  if (safe('sideOwnerHotel'))    safe('sideOwnerHotel').textContent    = user.hotel_name || user.hotelName || 'Your Property';
  if (safe('ownerGreetName'))    safe('ownerGreetName').textContent    = (user.name || 'Partner').split(' ')[0];
}
...
/* ── Properties (API-first, localStorage fallback) ── */
function getLocalProperties() {
  try {
    const s = localStorage.getItem('aurum-owner-properties');
    if (s) return JSON.parse(s);
  } catch (e) {}
  return [{ name: user?.hotel_name || user?.hotelName || 'Grand Hotel AURUM', city:'Paris', country:'France', stars:5, rooms:56, bookings:148, revenue:84200, occ:83, status:'live' }];
}
function saveLocalProperties(props) { localStorage.setItem('aurum-owner-properties', JSON.stringify(props)); }
function getLocalRooms() {
  try { const s = localStorage.getItem('aurum-owner-rooms'); if (s) return JSON.parse(s); } catch (e) {}
  return null;
}
function saveLocalRooms(rooms) { localStorage.setItem('aurum-owner-rooms', JSON.stringify(rooms)); }

let properties = getLocalProperties();

/* ── Fetch & render KPIs + recent bookings from API ── */
async function loadDashboardData() {
  ...
  try {
    ...
    if (data.success) {
      ...
      const kpiBookEl  = document.getElementById('kpiBookings');
      const kpiPropEl  = document.getElementById('kpiProperties');
      if (kpiRevEl)  kpiRevEl.textContent  = '$' + ((stats.total_revenue || 0)).toLocaleString();
      if (kpiBookEl) kpiBookEl.textContent = stats.total_bookings || 0;
      if (kpiPropEl) kpiPropEl.textContent = stats.properties || properties.length;

      // Render recent bookings from API
      const recentEl = document.getElementById('recentBookingsList');
      const recent   = data.data.recent_bookings || [];
      if (recentEl && recent.length) {
        recentEl.innerHTML = recent.slice(0, 5).map(b => `
          <div class="bl-item">
            ...
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;">
              ...
              <div class="bl-status ${b.status || 'confirmed'}">${b.status || 'confirmed'}</div>
            </div>
          </div>`).join('');
        return; // don't render mock
      }
    }
  } catch (e) {
    console.warn('Dashboard API unavailable, using mock data.', e.message);
  }
  renderMockRecentBookings();
}

/* ── Fetch & sync properties from API ── */
async function loadPropertiesFromAPI() {
  const token = localStorage.getItem('aurum-token');
  if (!token) { renderProperties(); return; }
  ...
  try {
    ...
    if (data.success && Array.isArray(data.data)) {
      if (data.data.length) {
        // Merge API data with local stats (bookings, occ, revenue come from analytics)
    ...
    console.warn('Properties API unavailable, using local data.', e.message);
  }
  renderProperties();
}

function renderMockRecentBookings() {
...
function renderProperties() {
  ...
  const kpiEl = document.getElementById('kpiProperties');
  if (kpiEl) kpiEl.textContent = properties.length;
}
...
function renderRoomGrid() {
  ...
  el.querySelectorAll('.room-cell').forEach(cell => {
    cell.addEventListener('click', () => {
      ...
      const nextIdx   = (ROOM_STATUSES.indexOf(roomData[idx].status) + 1) % ROOM_STATUSES.length;
...
initDashSelect('revPeriodWrap',     'revPeriod',     () => {});
...
window.openEditProp = function (index) {
  ...
  if (epmForm)          epmForm.style.display = 'block';
  if (epmDeleteConfirm) epmDeleteConfirm.classList.remove('active');
...
document.querySelectorAll('.star-opt').forEach(star => {
  star.addEventListener('click', () => {
    ...
    if (document.getElementById('epmStarVal')) document.getElementById('epmStarVal').value = star.dataset.val;
  });
});

document.getElementById('epmDeleteBtn')?.addEventListener('click', () => {
  if (epmForm)          epmForm.style.display = 'none';
  if (epmDeleteConfirm) epmDeleteConfirm.classList.add('active');
});
...
document.getElementById('epmConfirmDeleteBtn')?.addEventListener('click', async () => {
  const idx  = parseInt(document.getElementById('epmPropIndex').value);
  const prop = properties[idx];
  if (!prop) return;

  // Try API delete if we have an id
  const token = localStorage.getItem('aurum-token');
  if (token && prop.id) {
    try {
      await fetch(`${API_BASE}/owner/properties/${prop.id}`, {
        method:  'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
...
epmForm?.addEventListener('submit', async e => {
  e.preventDefault();
  const idx     = parseInt(document.getElementById('epmPropIndex').value);
  const updated = {
    ...properties[idx],
  ...
  // Try API update if we have an id
  const token = localStorage.getItem('aurum-token');
  if (token && updated.id) {
    try {
      await fetch(`${API_BASE}/owner/properties/${updated.id}`, {
        method:  'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body:    JSON.stringify({ name: updated.name, city: updated.city, country: updated.country, stars: updated.stars, rooms: updated.rooms }),
      });
    } catch (e) { console.warn('Update API error', e); }
  }
...
/* ── Init ── */
loadDashboardData();         // KPIs + recent bookings from API
loadPropertiesFromAPI();     // Properties from API, falls back to localStorage
renderRevChart();
renderBookingsTable();
renderRoomGrid();
renderRevBreakdown();
