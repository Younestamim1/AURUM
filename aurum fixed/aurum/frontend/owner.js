...
themeToggle?.addEventListener('click', () => {
  ...
  if (currentStars) setStarRating(parseInt(currentStars));
});
...
function goStep(n) {
  ...
  currentStep = n;
  window.scrollTo({ top: (document.getElementById('listingForm')?.offsetTop || 0) - 80, behavior: 'smooth' });
  updateCompleteness();
}
...
function addRoomType(name = 'Deluxe Room', price = '', capacity = 2, qty = 10) {
  ...
  updatePreview(); updateCompleteness();
}
...
document.querySelectorAll('.comm-card').forEach(card => {
  card.addEventListener('click', () => {
    ...
    if (document.getElementById('f-commission')) document.getElementById('f-commission').value = card.dataset.val;
    if (document.getElementById('opbComm'))      document.getElementById('opbComm').textContent = card.dataset.val + '%';
  });
});
...
function handleFiles(files) {
  [...files].forEach(file => {
    ...
    reader.onload = ev => {
      ...
      thumb.id = `pt-${pc}`;
      thumb.innerHTML = `<img src="${ev.target.result}" alt="Photo ${pc}"/><button class="photo-thumb-remove" onclick="removePhoto(${pc})">✕</button><div class="photo-thumb-label">${photoLabels[pc-1] || 'Photo ' + pc}</div>`;
      photoGrid?.appendChild(thumb);
      updateCompleteness();
      updatePreviewImage(ev.target.result);
    };
...
let   previewTimer;
function flashUpdating() {
  previewUpdating?.classList.add('show');
  clearTimeout(previewTimer);
  previewTimer = setTimeout(() => previewUpdating?.classList.remove('show'), 800);
}
...
function updatePreview() {
  ...
  if (document.getElementById('prevInitials')) document.getElementById('prevInitials').textContent = name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2) || '—';
  const city    = document.getElementById('f-city')?.value    || 'City';
  const country = document.getElementById('f-country')?.value || 'Country';
  if (document.getElementById('prevLocation')) document.getElementById('prevLocation').innerHTML = `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg> ${city}, ${country}`;
  const desc = document.getElementById('f-desc')?.value || 'Your hotel description will appear here…';
  if (document.getElementById('prevDesc'))  document.getElementById('prevDesc').textContent  = desc;
  const stars = parseInt(document.getElementById('f-stars')?.value) || 0;
  if (document.getElementById('prevBadge')) document.getElementById('prevBadge').textContent = stars ? `${stars} ★` : '— ★';
  if (document.getElementById('prevStars')) document.getElementById('prevStars').textContent = '★'.repeat(stars) + '☆'.repeat(5 - stars);
  ...
  if (document.getElementById('prevPrice')) document.getElementById('prevPrice').textContent = prices.length ? '$' + Math.min(...prices) : '$—';
  ...
  if (document.getElementById('prevAmenities')) document.getElementById('prevAmenities').innerHTML = checked.map(a => `<span class="pa-tag">${a}</span>`).join('');
  const ownerName  = document.getElementById('f-ownername')?.value || '—';
  const ownerEmail = document.getElementById('f-email')?.value     || '—';
  if (document.getElementById('opbName'))  document.getElementById('opbName').textContent  = ownerName;
  if (document.getElementById('opbEmail')) document.getElementById('opbEmail').textContent = ownerEmail;
  updateCompleteness();
}

function updatePreviewImage(src) {
  const placeholder = document.getElementById('prevImgPlaceholder');
  if (placeholder) placeholder.innerHTML = `<img src="${src}" style="width:100%;height:100%;object-fit:cover;" alt="Hotel"/>`;
}
...
['f-name','f-city','f-country','f-address','f-desc','f-ownername','f-email','f-type'].forEach(id => {
...
function updateCompleteness() {
  const checks = [
    ...
    { id: 'ci-desc',     done: (document.getElementById('f-desc')?.value.trim().length || 0) >= 30 },
  ...
  if (document.getElementById('compPct')) document.getElementById('compPct').textContent = pct + '%';
  if (document.getElementById('compBar')) document.getElementById('compBar').style.width = pct + '%';
  checks.forEach(c => {
    const el = document.getElementById(c.id);
    if (el) { el.classList.toggle('done', c.done); el.classList.toggle('pending', !c.done); }
  });
}
...
document.querySelectorAll('.num-count').forEach(el => counterObs.observe(el));

/* ══════════ SUBMIT — real API call ══════════ */
document.getElementById('finalSubmitBtn')?.addEventListener('click', async () => {
  if (!document.getElementById('agreeTerms')?.checked) {
    showToast('Please agree to the Partner Terms & Conditions.', 'error'); return;
  }

  // Run all step validations
  for (let s = 1; s <= 4; s++) {
    const err = validateStep(s);
    if (err) { showToast(err, 'error'); goStep(s); return; }
  }
  ...
  if (!token) {
    showToast('Please log in as an owner first. Redirecting…', 'error');
  ...
  try {
    ...
    if (data.success) {
      ...
      const nameEl = document.getElementById('successHotelName');
